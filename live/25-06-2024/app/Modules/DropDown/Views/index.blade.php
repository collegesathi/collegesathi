                ...
                <table class="table table-bordered table-striped table-hover "  >
                    <thead>
                        <tr role="row">
							...
							@endIf
                           <th width="10%">
							{{	link_to_route("$modelName.index",trans("messages.$modelName.$type.name"),array(
									$type,'records_per_page' => $recordPerPage,'sortBy' => 'name','order' => ($sortBy == 'name' && $order == 'desc') ? 'asc' : 'desc') + $searchVariable,
                    ...
                    <tbody id="powerwidgets">
						...
						<td data-th='{{ trans("messages.global.action") }}'>
						 ...
						 @if($record->status)
                            <a class="text-decoration-none confirm_box"   data-href='{{ route("$modelName.status",array($record->id,INACTIVE,$type))}}'  class="text-decoration-none confirm_box" data-confirm-message="{{trans('messages.admin.system.you_want_to_perform_this_action')}}" data-confirm-heading="{{trans('messages.admin.system.are_you_sure')}}" >
