...
@section('breadcrumbs')
	{{ Breadcrumbs::render('state-list', $countryName) }}
@stop

@section('content')
<script type="text/javascript">
 var action_url = '<?php echo route("State.Multipleaction"); ?>';
...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        {{ Form::open(['method' => 'get','role' => 'form',route("State.index","$countryId"),'class' => 'mws-form']) }}
                        ...
                        <div class="row clearfix ">
                            ...
                            <div class="col-xs-12 col-sm-6 col-md-4 col-lg-6">
                                <div class="form-group">
                                    <div class="form-line">
										{{ Form::text('state_name',((isset($searchVariable['state_name'])) ? $searchVariable['state_name'] : ''), ['class' => 'form-control','placeholder'=>trans("messages.State.state_name")]) }}
                                    </div>
                                </div>
                            </div>
                           <div class="col-xs-12 col-sm-6 col-md-4 col-lg-6">
...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="header">
                <h2>
                   {{ trans("messages.State.table_heading_index") }}
                    <ul class="header-dropdown m-r--5 btn-right-top-margin visible-md visible-lg visible-sm visible-xs">
                       <li>
                            <a href="{{ route('State.add',$countryId) }}" >
                                <button type="button" class="btn bg-indigo waves-effect">
                                    <i class="material-icons font-14">add</i> {{ trans("messages.State.add_new") }}
                                </button>
                            </a>
			 ...
			 <div class="body table-responsive">
                ...
                <table class="table table-bordered table-striped table-hover "  >
					...
					<th>
						{{	link_to_route('State.index',trans("messages.State.state_name"),	array(					 $countryId,'sortBy' => 'state_name','order' => ($sortBy == 'state_name' && $order == 'desc') ? 'asc' : 'desc') + $searchVariable,
							array('class' => (($sortBy == 'state_name' && $order == 'desc') ? 'sorting desc' : (($sortBy == 'state_name' && $order == 'asc') ? 'sorting asc' : 'sorting')) ))
						}}
					</th>
					<th>

                        {{	link_to_route('State.index',trans("messages.State.state_code"),	array(					 $countryId,'sortBy' => 'state_code','order' => ($sortBy == 'state_code' && $order == 'desc') ? 'asc' : 'desc') + $searchVariable,
							array('class' => (($sortBy == 'state_code' && $order == 'desc') ? 'sorting desc' : (($sortBy == 'state_code' && $order == 'asc') ? 'sorting asc' : 'sorting')) ))
						}}


                    </th>
					<th>{{ trans("messages.Country.country_name") }}
                    </th>
                    ...
                    <tbody id="powerwidgets">
					   ...
					   @foreach($result as $record)
						<tr class="items-inner">
							...
							<td data-th='{{ trans("messages.State.state_code") }}'>
							...
							<td data-th='{{ trans("messages.global.action") }}'>
								 ...
								 <a href='{{ route( "State.edit",array($countryId,$record->id))}}' class="text-decoration-none">
								...
								@if($record->status == 1)
										...
										<a class="text-decoration-none confirm_box"   data-href='{{ route("State.status",array($record->id,INACTIVE))}}'  class="text-decoration-none confirm_box" data-confirm-message="{{trans('messages.admin.system.you_want_to_perform_this_action')}}" data-confirm-heading="{{trans('messages.admin.system.are_you_sure')}}" >
								...
								@else
									 <a class="text-decoration-none confirm_box"  data-href='{{ route("State.status",array($record->id,ACTIVE) )}}'  class="text-decoration-none confirm_box" data-confirm-message="{{trans('messages.admin.system.you_want_to_perform_this_action')}}" data-confirm-heading="{{trans('messages.admin.system.are_you_sure')}}">
								...
								<a href="{{ route('City.index',$record->id) }}" class="btn btn-info btn-small">{{ trans("messages.State.cities") }} </a>
