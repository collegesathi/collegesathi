...
$(document).ready(function() {
    ...
    /* Refer A Friend */
    if ($('#referAFriendForm').length > 0) {
        openReferAFriendFormValidations();
    }
 ...
 *  Function : validation for change password form
 */
function openReferAFriendFormValidations() {
    $('#referAFriendForm').formValidation('destroy', true);
    $('#referAFriendForm').formValidation({
        message: 'This value is not valid',
        trigger: 'blur',
        fields: {
            'email': {
				row: '.form-group',
				err: '.user_Refer_email_error',
				validators: {
					notEmpty: {
						message: ERROR_ENTER_EMAIL_ADDRESS
					},
        ...
        e.preventDefault();
        referAFriend('referAFriendForm', refer_promocode_url);
    });

    $('#referAFriendForm').formValidation('resetForm', { reset: true });
}
