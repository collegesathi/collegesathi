...
$source 			= 	isset($_REQUEST['source']) ? $_REQUEST['source'] : "Amity MBA";
	...
	if ($name != "" && $email != "" && $contactno != "" && strlen($contactno) >= 10 && $city != "" && $conn== TRUE){
		...
		elseif(!preg_match($reg_three,$contactno)){
            ...
            $request = array(
				...
				array("Attribute" => 'mx_online_Name', "Value" => $university_name),
            ...
            curl_close($curl);
           echo $jsonResult = json_decode($response, true);
			

			if($from_fb == true){
				$subject = "New Amity Online MBA Social Media Lead";
			}
			else{
				$subject = "New Amity Online MBA Lead";
			}
			...
			$message .= "Source : New Amity MBA  <br/>";
