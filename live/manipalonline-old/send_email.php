...
$source 			= 	isset($_REQUEST['source']) ? $_REQUEST['source'] : "Landing Page Manipal";
...
if(isset($_POST) && !empty($_POST)){
	...
	if ($name != "" && $email != "" && $contactno != "" && strlen($contactno) >= 10 && $state != "" && $course != "" && $conn== TRUE){
		...
		elseif(!preg_match($reg_three,$contactno)){
			...
			$university_name	=	'Manipal University';
			$mx_Website_Page	=	'Manipal University';
			...
			$request = array(
                ...
                array("Attribute" => 'mx_Course_Applying_For', "Value" => $course),
                array("Attribute" => 'mx_State', "Value" => $state),
			...
			if($from_fb == true){
				$subject = "Manipal Online Social Media Lead";
			}
			else{
				$subject = "Manipal Online Lead";
			}
			...
			$message .= "Source : Manipal Online  <br/>";
