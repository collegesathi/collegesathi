...
$source = isset($_REQUEST['source']) ? $_REQUEST['source'] : "sharda University";
...
if (isset($_POST) && !empty($_POST)) {
	...
	if ($name != "" && $email != "" && $contactno != "" && strlen($contactno) >= 10 && $city != "" && $conn == TRUE) {
		...
		if (empty($contactno)) {
			...
			if ($from_fb == true) {
				$subject = "Sharda Online MBA Social Media Lead";
			} else {
				$subject = "Sharda Online MBA Lead";
			}
			...
			$message .= "Source : Sharda Online  <br/>";
			...
			echo $sql = "INSERT INTO `leads` (`type`,`name`, `email`, `contactno`, `program`,`leadtype`, `admission`,`qualification`,`specialization`,`university`,`city`,`ipaddress`,`source`,`source_campaign`,`source_medium`,`mx_Website_Page`,`lsq_response`, `created_at`)VALUES ('$pageurl','$name', '$email', '$contactno', '','$leadtype', '','$qualification','$specialization','$university_name','$city','$ipaddress','$source','$source_campaign','$source_medium','$mx_Website_Page','$response','$created_at')";
