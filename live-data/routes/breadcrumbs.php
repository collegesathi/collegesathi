...
Breadcrumbs::for('semester', function (BreadcrumbTrail $trail,$univercityId,$course_id,$specification_id) {
    $trail->parent('course',$univercityId);
    if(!empty($course_id) && !empty($specification_id)){
        $specificationName = CustomHelper::getMasterDropdownNameById($specification_id);
        $trail->push($specificationName,route('Course.courseSpecification',[$course_id,$univercityId,$specification_id]));
    }
