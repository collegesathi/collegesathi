...
function getState(country_id, stateName, cityName, stateDiv, cityDiv, formId, selected_value = '', classname = '', bankDiv) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#' + cityDiv).empty();
            $('#' + cityDiv).append('<select class="autocomplete form-select show-tick' + classname + '" name="' + city_field_name + '" id="' + city_field_name + '"><option value=""> Please Select City </option></select>');
            ...
            if (formId != '') {
                ...
                $('#' + formId).formValidation('addField', city_field_names);
            }
        }
    });
}
