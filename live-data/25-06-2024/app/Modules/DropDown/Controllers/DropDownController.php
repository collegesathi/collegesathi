...
class DropDownController extends BaseController {
	...
	*/
	public function listDropDown($type=''){
		...
		return  View::make("DropDown::index",compact('recordPerPage','model','searchVariable','sortBy','order','type'));
	}// end listDropDown()
	...
	*/
	public function addDropDown($type=''){
		...
		return  View::make("DropDown::add",compact('type','languages','language_code', 'pageTitle'));
	} //end addDropDown()
	...
	*/
	public function editDropDown($Id,$type){
		...
		if(!empty($modelDescriptions)){
 		...
 		return  View::make("DropDown::edit",array('model' => $dropdown,'type'=>$type, 'languages' => $languages, 'language_code' => $language_code, 'multiLanguage' => $multiLanguage, 'pageTitle' => $pageTitle ));
	}// end editDropDown()
	...
	/**
		...
		* @return redirect page.
	*/
	function updateDropDown($modelId,$type=''){
