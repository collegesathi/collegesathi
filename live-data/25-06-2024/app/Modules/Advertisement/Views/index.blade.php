...
@section('content')
@section('breadcrumbs')
	{{ Breadcrumbs::render('advertisement-page-list') }}
@stop
...
<table class="table table-bordered table-striped table-hover "  >
	<thead>
		<tr role="row">
			...
			<th  width="15%" class="{{(($sortBy == 'title' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'title' && $order == 'asc') ? 'sorting_asc' : 'sorting')) }} ">
				{{ link_to_route("Advertisement.index", trans("messages.global.title"),array('records_per_page' => $limit,'sortBy' => 'title','order' => ($sortBy == 'title' && $order == 'desc') ? 'asc' : 'desc')+ $searchVariable, array('class' => (($sortBy == 'title' && $order == 'desc') ? 'sorting desc' : (($sortBy == 'title' && $order == 'asc') ? 'sorting asc' : 'sorting')) ) )}}
			</th>
			<th width="15%">{{ trans("messages.global.display_location") }}</th>
			<th width="10%">{{ trans("messages.global.status") }}</th>
			<th>
				{{ link_to_route("Advertisement.index",trans("messages.global.created"),array('records_per_page' => $limit,'sortBy' => 'created_at','order' => ($sortBy == 'created_at' && $order == 'desc') ? 'asc' : 'desc')+ $searchVariable, array('class' => (($sortBy == 'created_at' && $order == 'desc') ? 'sorting desc' : (($sortBy == 'created_at' && $order == 'asc') ? 'sorting asc' : 'sorting')) )) }}
			</th>
			...
			@foreach($model as $record)
				<tr class="items-inner">
                    ...
                    <td data-th='{{ trans("messages.$modelName.small_image") }}'>
                        ...
                        @if ($record->image != '' && File::exists(ADVERTISEMENT_IMAGE_ROOT_PATH . $record->image))

                            @php  echo CustomHelper::showUserImage(ADVERTISEMENT_IMAGE_ROOT_PATH, ADVERTISEMENT_IMAGE_URL,  $record->image,  $record->gender, ['alt' => $record->full_name, 'height' =>'60', 'width' =>'60']);  @endphp
					...
					<td data-th='{{ trans("messages.$modelName.location") }}'>{{ CustomHelper::getConfigValue('DISPLAY_PAGE.'.$record->location) }}</td>
