...
class timthumb {
	...
	public function __construct(){
		...
		$imageType = 1;
		if($this->param('webp', 0) && false != strpos($_SERVER['HTTP_ACCEPT'], 'image/webp')){
			$imageType = 2;
		}
		if($this->param('avif', 0) && false != strpos($_SERVER['HTTP_ACCEPT'], 'image/avif')){
			$imageType = 3;
		}
		...
		if($this->isURL){
			...
			asort($arr);
			$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . implode('', $arr) . $imageType . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
		} else {
			...
			$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . $this->localImageMTime . $_SERVER ['QUERY_STRING'] . $imageType . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
		}
	...
	protected function processImageAndWriteToCache($localImage){ 
		...
		$convert_webp = (int) abs ($this->param('webp', 0));
		$convert_w = (int) abs ($this->param('w', 0));
		$convert_avif = (int) abs ($this->param('avif', 0));
		$zoom_crop = (int) $this->param('zc', DEFAULT_ZC);
		$quality = (int) abs ($this->param('q', DEFAULT_Q));
		$align = $this->cropTop ? 't' : $this->param('a', 'c');
		$filters = $this->param('f', DEFAULT_F);
		$sharpen = (bool) $this->param('s', DEFAULT_S);
		$canvas_color = 'fff';//$this->param('cc', DEFAULT_CC);
		$canvas_trans = (bool) $this->param('ct', '1');
		//echo $convert_w."<hr>";
		//echo $convert_webp."<hr>";
		...
		// Convert image to webp { Check browser support webp or not }
		if( $convert_webp == 1){
			$imgType = 'web';
			imagewebp($canvas, $tempfile, $quality);
		} else if( $convert_avif == 1){  
			$imgType = 'avi';
			imageavif($canvas, $tempfile, $quality);
		} else if(preg_match('/^image\/(?:jpg|jpeg)$/i', $mimeType)){ 
			$imgType = 'jpg';
	...
	protected function sendImageHeaders($mimeType, $dataSize){
		...
		if(strtolower($mimeType) == 'image/web'){
			$mimeType = 'image/webp';
		}
		if(strtolower($mimeType) == 'image/avi'){
			$mimeType = 'image/avif';
		}
