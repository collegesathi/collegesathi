...
class BlogService
{
    ...
    public static function BlogValidateandSave($formData = array(), $attribute = array()){
        ...
        if ($validator->fails()) {
            ...
            if (isset($formData['image']) && !empty($formData['image'])) {
                ...
                if ($formData['image']->move(BLOG_IMAGE_ROOT_PATH, $fileName)) {
                    $old_image = $obj->image;
                ...
                if($type == 'edit') BlogDescription::where('parent_id', $modelId)->delete();
                ...
                foreach ($formData['data'] as $langCode => $descriptionResult) {
                    ...
                    if ($descriptionResult == CustomHelper::getConfigValue('defaultLanguageCode')) {
                        $modelDescription                = new BlogDescription();
    ...
    public function getBlogDetail($formData = array(), $attribute = array()){
        ...
        $sortBy                         = (isset($formData['sortBy'])) ? $formData['sortBy'] : 'id';
        ...
        $uaeResult       = Blog::where('product_type',UAE_TOURIST_VISA_ID)->with('descriptionData')->where('is_active', ACTIVE)->orderBy($sortBy, $order)->paginate($recordPerPage);

        $travelResult    = Blog::where('product_type',TRAVEL_INSURANCE_ID)->with('descriptionData')->where('is_active', ACTIVE)->orderBy($sortBy, $order)->paginate($recordPerPage);

        $comboResult     = Blog::where('product_type',COMBO_ID)->with('descriptionData')->where('is_active', ACTIVE)->orderBy($sortBy, $order)->paginate($recordPerPage);

        $status                         = SUCCESS;
        $response['status']             = $status;
        $response['sort_by']            = $sortBy;
        $response['order']              = $order;
        $response['uae_blog']           = $uaeResult;
        $response['uae_last_page']      = $uaeResult->lastPage();
        $response['travel_blog']        = $travelResult;
        $response['travel_last_page']   = $travelResult->lastPage();
        $response['combo_blog']         = $comboResult;
        $response['combo_last_page']    = $comboResult->lastPage();
        ...
        //         $searchData['title'] = trim($searchData['keyword']);
        ...
        //                     $query->where('title', 'like', '%' . $fieldValue . '%')->orWhere('description', 'like', '%' . $fieldValue . '%');
        //                 });
        //             }
        //         }
        ...
        // $order                          = (isset($formData['order'])) ? $formData['order'] : 'DESC';
        // $recordPerPage                  = (isset($formData['records_per_page']) && $formData['records_per_page'] != '') ? $formData['records_per_page'] : Config::get("Reading.records_per_page");
        // $result                         = $DB->orderBy($sortBy, $order)->paginate($recordPerPage);
        // $status                         = SUCCESS;
        // $response['sort_by']            = $sortBy;
        // $response['order']              = $order;
    ...
    public function viewBlogData($formData = array(), $attribute = array())
    {
        ...
        if (!empty($formData['slug'])) {
            $slug = $formData['slug'];
            $record = Blog::with(['descriptionData'])->where('slug', $slug)->firstOrFail();
            $status = SUCCESS;
        }
