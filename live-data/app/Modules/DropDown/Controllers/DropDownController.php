	...
	*/
	function saveDropDown($type='', $id = ''){
		...
		if ($validator->fails()){
			...
			$model->status	    			= ACTIVE;
			if($type == 'course'){
				$model->show_on_footer	= (isset($thisData['show_on_footer']) && !empty($thisData['show_on_footer'])) ? $thisData['show_on_footer'] : INACTIVE;
			}
			$model->save();
			$modelId				=	$model->id;
	...
	*/
	function updateDropDown($modelId,$type='', $dropdown_id = ''){
		...
		if ($validator->fails())
		{
			...
			if($type == 'course'){
				$model->show_on_footer	= (isset($this_data['show_on_footer']) && !empty($this_data['show_on_footer'])) ? $this_data['show_on_footer'] : INACTIVE;
			}
			$model->save();
