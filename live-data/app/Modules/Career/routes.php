...
Route::group(array('prefix'=>ADMIN_ROUTE_PREFIX,'namespace'=>'App\Modules\Career\Controllers'), function()
{
	Route::group(array('middleware' => ['web','App\Http\Middleware\PreventBackHistory','App\Http\Middleware\AuthAdmin']), function() {
		...
		Route::get('/job-requests/{type}/{career_id?}',array('as'=>'Career.job_requests','uses'=>'JobRequestController@jobRequests'));
        Route::get('/view-job-request/{type}/{id}/{career_id?}',array('as'=>'Career.view_job_request','uses'=>'JobRequestController@viewJobRequest'));
        Route::any('/download-cv-document/{id}/{file}',array('as'=>'Career.download_cv','uses'=>'JobRequestController@downloadCV'));
	});
