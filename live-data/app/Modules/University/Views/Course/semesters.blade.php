...
@section('breadcrumbs')
    {{ Breadcrumbs::render('semester', $uni_id,$course_id,$specification_id) }}
@stop
...
<div class="d-flex flex-end clearfix Semesters-head">
    <div class="flex-btns">
        @if (isset($uni_id) &&
                !empty($uni_id) &&
                (isset($course_id) && !empty($course_id)) &&
                (isset($specification_id) && !empty($specification_id)))
            <a href='{{ route('Course.courseSpecification', [$course_id, $uni_id, $specification_id]) }}'>
    ...
    @if ($acitveSemesters > 0)
        <div class="flex-btns">
            <a href='{{ route("$modelName.markAsAllInactive", [$course_id, $uni_id, $specification_id]) }}'>
                <button type="button"
                    class="btn bg-indigo waves-effect">{{ trans('messages.global.mark_as_all_in_active') }}</button>
            </a>
        </div>
    @endif
    @if ($inacitveSemesters > 0)
        <div class="flex-btns">
            <a href='{{ route("$modelName.markAsAllActive", [$course_id, $uni_id, $specification_id]) }}'>
                <button type="button"
                    class="btn bg-indigo waves-effect">{{ trans('messages.global.mark_as_all_active') }}</button>
            </a>
        </div>
    @endif
</div>

@for ($i = 0; $i < $noOfSemesters[0]; $i++)
        @php
            $sessionValue = Session::get('semester');
            $getSemesterData = CustomHelper::getSemesterData($uni_id, $course_id, $i + 1, $specification_id);

            if(!empty($getSemesterData->toArray())){
                $color = 'green';
            } else{
                $color = 'grey';
            }
    ...
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
        <div class="panel-group" id="panel-group-id" role="tablist" aria-multiselectable="true">
            <div class="panel" style="color: #fff; border:1px solid {{$color}};">
                <div class="panel-heading" role="tab" id="panel-heading-id">
                    <h4 class="panel-title">
                        <a role="button" data-toggle="collapse" data-target="#panel-collapse-id{{ $i }}"
                            data-parent="#panel-group-id" href="javascript:void(0)" aria-expanded="true"
                            aria-controls="panel-collapse-id"
                            class=" bg-{{$color}} {{ $sessionValue == $i + 1 ? '' : 'collapsed' }}">{{ trans('messages.global.semester_heading') . ' ' . ($i + 1) }}
                ...
                <div id="panel-collapse-id{{ $i }}"
                    class="panel-collapse collapse {{ $sessionValue == $i + 1 ? 'in' : '' }}" role="tabpanel"
                    aria-labelledby="panel-heading-id" aria-expanded="true">
                                ...
                                <div class="card">
                                    ...
                                    <div class="row clearfix">
                                        <div class="col-xs-12 col-sm-6 col-md-4 pull-right set_button"
                                            ...
                                            <button type="button"
                                                class="btn bg-green btn-sm waves-effect btn-submit text-decoration-none submit_btn"
                                                data-semester_key="{{ $i }}">{{ trans('messages.global.submit') }}</button>
                                        </div>
                                    </div>
                                    {{ Form::close() }}
                                </div>
...
<div class="modal fade" id="subject_detail_view_model" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
        ...
        <div class="modal-content">
                ...
                <table class="table table-bordered table-striped table-hover ">
                    <thead>
                        <tr role="row">
                            ...
                            <th width="15%"> {{ trans('messages.global.credit_score') }} </th>
                            <th width="65%"> {{ trans('messages.global.description') }} </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td id="subject"></td>
                            <td id="credit_score"></td>
                            <td class="semester_desripriton" id="description"></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary"
                    data-dismiss="modal">{{ trans('messages.global.close') }}</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.view_subject_detail', function() {
            ...
            var credit_score = $(this).data('credit_score');
            $("#subject_detail_view_model").modal('show');
            $("#subject").html(subject);
            $("#description").html(description);
            $("#credit_score").html(credit_score);
        });
