                                        ...
                                        <div class="course_details card">
                                            <div class="otheruniversities_img">
                                                <figure class="university_logo_explore_program">
                                                    <?php
                            if (file_exists(UNIVERSITY_IMAGE_ROOT_PATH . $item->getAllUniversityDetails->image) && !empty($item->getAllUniversityDetails->image)) {
                                echo '<img src="' . UNIVERSITY_IMAGE_URL . $item->getAllUniversityDetails->image . '" alt="University Logo" />';
                            }
                                                    ?>
                                                </figure>
                                            </div>
                                                ...
                                                <div class="student_rating d-flex">
                                                    <div class="top_rating d-flex">
                                                        <span class="rating d-flex"
                                                            data-score='{{$item->getAllUniversityDetails->avg_rating}}'></span>
                                                    </div>
                                                    <span>( {{count($item->getAllUniversityDetails->getReviewRatingUniversities)}}
                                                        {{trans('front_messages.global.reviews')}})</span>
                                                </div>
                                            </div>
                                            <div class="card-footer other_universities">
                                                <a href="javascript:void(0);" class="btn btn-primary addToCompare"
                                                    data-university_id="{{ $item->univercity_id }}" data-course_id="{{ $item->course_id }}"
                                                    data-page_request="course_university"
                                                    id="addToCompare{{$item->univercity_id}}">{{ $text }}</a>
                                            </div>
                                        </div>
                                    </div>
                        @endforeach
