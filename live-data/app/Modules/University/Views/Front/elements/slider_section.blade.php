    ...
    <div class="container">
        <div class="programs_banner_content d-flex">
            <div class="programs_details">
                <?php
                $routeWithSlug = route('University.frontIndex',$slug);
                $base64Route = base64_encode($routeWithSlug);
                
                
                if(empty($specificationDetails)){
                    ...
                    $name = $specificationDetails->getCourseSpecificationDropdownDetails->name . ' - ' . $courseDetail->name . ' - ' .  $result->title;
                }
                
                ?>
                <div class="programs_details_contant">
                    <h1>{{ $name }}</h1>
                    <div class="certificate_listing uni-shor-des">
                    ...
                    <div class="compare_btn_box">
                        ...
                        <div class="d-flex addto_compare_box">
                            <div class="compare_btn">
                                <a href="{{ route('User.login',['redirect' => $base64Route]) }}">
                                    <img class="plus_icon" src="{{ WEBSITE_IMG_URL }}plus_icon.svg" alt="img">
                                    <img class="plus_icon_white" src="{{ WEBSITE_IMG_URL }}plus_icon_white.svg" alt="img">{{ trans('front_messages.global.review') }}</a>
                            </div>
                        </div>
                        ...
                        <div class="program_img_box">
                            ...
                            <div class="small_logo">
                                <figure class="university_logo_explore_program">
                                    @if ($result->image != '' && File::exists(UNIVERSITY_IMAGE_ROOT_PATH . $result->image))
                                    <img src="{{ UNIVERSITY_IMAGE_URL . $result->image}}" alt="image">
                                    @endif
                                    <?php
