...
class BlogService
{
    ...
    public function getBlogDetail($formData = array(), $attribute = array()){
        ...
        $result       = Blog::where('university_id',$uni_id)->with('descriptionData')->where('is_featured',INACTIVE)->where('is_active', ACTIVE)->orderBy($sortBy, $order)->paginate($recordPerPage);

        $trending_posts     = Blog::where('university_id',$uni_id)->with('descriptionData')->where('is_active', ACTIVE)->limit(10)->orderBy($sortBy, $order)->get();

        $featured_posts     = Blog::where('university_id',$uni_id)->where('is_featured',ACTIVE)->with('descriptionData')->where('is_active', ACTIVE)->orderBy('updated_at', $order)->get();
        ...
        $response['trending_posts']    = $trending_posts;
