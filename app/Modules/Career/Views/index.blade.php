                        ...
                        <div class="row clearfix ">
                            <div class="col-xs-12 col-sm-6 col-md-4 col-lg-6">
                                <div class="form-group">
                                    <div class="form-line">
                                        {{ Form::text('job_title', isset($searchVariable['job_title']) ? $searchVariable['job_title'] : '', ['class' => 'form-control', 'placeholder' => trans('messages.global.filter_by_job_name')]) }}
                            ...
                            <div class="col-xs-12 col-sm-6 col-md-4 col-lg-6">
                                <div class="form-group">
                                    <div class="form-line">
                                        {{ Form::select('job_type', ['' => trans('messages.global.filter_by_job_type')] + $job_type, isset($searchVariable['job_type']) ? $searchVariable['job_type'] : '', ['class' => 'form-control', 'data-live-search' => 'true', 'id' => 'job_type']) }}
                                    </div>
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-6 col-md-4 col-lg-6">
        ...
        <div class="card">
            ...
            <div class="body table-responsive">
                ...
                <table class="table table-bordered table-striped table-hover ">
                    <thead>
                        <tr role="row">
                            <th> {{ trans('messages.global.job_title') }} </th>
                            <th> {{ trans('messages.global.job_type') }} </th>
                            <th>{{ trans("messages.global.total_applications") }}</th>
                        ...
                        @if (!$model->isEmpty())
                            @foreach ($model as $record)
                                <tr class="items-inner">
                                    <td data-th="{{ trans('messages.global.job_title') }}">
                                        {{ $record->job_title }}
                                    </td>
                                    <td data-th="{{ trans('messages.global.job_type') }}">
                                        {{ Config::get('JOB_TYPE.' . $record->job_type) }}
                                    </td>

                                    <td data-th="{{ trans('messages.global.job_type') }}"><a href='{{route("Career.job_requests",[LIST_TYPE,$record->id])}}' target="_blank">{{ count($record->getTotalApplications) }}</a></td>
                                    ...
                                    <td data-th="{{ trans('messages.global.action') }}">
                                        <div class="btn-group m-l-5 m-t-5">
                                            ...
                                            <ul class="dropdown-menu  min-width-220">
                                                <li>
                                                    <a href="{{ route('Career.edit_career', $record->id) }}"
                                                        class="waves-effect waves-block confirm_box">
                                                        <i class="material-icons">edit</i><?php echo trans('messages.global.edit'); ?>
                                                    </a>
                                                </li>
                                                <li>
                                                    <a href="{{ route('Career.view_career', $record->id) }}"
                                                        class="waves-effect waves-block confirm_box">
