...
Route::group(array('prefix'=>ADMIN_ROUTE_PREFIX,'namespace'=>'App\Modules\DropDown\Controllers'), function()
{
	Route::group(array('middleware' => ['web','App\Http\Middleware\PreventBackHistory','App\Http\Middleware\AuthAdmin']), function() {
		...
		Route::get('dropdown-manager/{type}/{id}/specifications',array('as'=>'DropDown.courseSpecifications','uses'=>'DropDownController@listDropDown'));
		Route::get('dropdown-manager/add-dropdown/{type}/{id}/specifications',array('as'=>'DropDown.addSpecification','uses'=>'DropDownController@addDropDown'));
		Route::post('dropdown-manager/add-dropdown/{type}/{id}/specifications',array('as'=>'DropDown.saveSpecification','uses'=>'DropDownController@saveDropDown'));
		Route::get('dropdown-manager/edit-dropdown/specifications/{id}/{type}/{dropdown_id}',array('as'=>'DropDown.editSpecification','uses'=>'DropDownController@editDropDown'));
		Route::post('dropdown-manager/edit-dropdown/specifications/{id}/{type}/{dropdown_id}',array('as'=>'DropDown.updateSpecification','uses'=>'DropDownController@updateDropDown'));
	});  
